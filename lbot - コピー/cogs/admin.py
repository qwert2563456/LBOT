...
from models import SystemConfig, get_or_create_user, get_p2p_fee_rate, get_escrow_fee_rate, User, Ledger
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                if not config:
                    config = SystemConfig(
                        collected_fees_ltc=Decimal("0"),
                        p2p_fee_percent=fee_decimal
                    )
                    session.add(config)
                else:
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                if not config:
                    config = SystemConfig(
                        collected_fees_ltc=Decimal("0"),
                        p2p_fee_percent=Decimal("0.002"),
                        escrow_fee_percent=fee_decimal
                    )
                    session.add(config)
                else:
...
class ViewUserModal(discord.ui.Modal, title="ユーザー情報確認"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with AsyncSessionLocal() as session:
            # Bug修正: get_or_create_user はユーザーを新規作成してしまうため
            # 管理者の参照用途では既存ユーザーのみ取得する
            db_user = await session.get(User, str(uid_int))
            if not db_user:
                await interaction.followup.send(
                    f"ユーザー `{uid_str}` はまだシステムに登録されていません。",
                    ephemeral=True
                )
                return

            available = quantize_ltc(db_user.available_balance or Decimal("0"))
            locked    = quantize_ltc(db_user.locked_balance or Decimal("0"))
            unconf    = quantize_ltc(db_user.unconfirmed_balance or Decimal("0"))
            total     = available + locked + unconf
        ...
        embed.add_field(name="未確定 LTC",     value=f"`{unconf:.8f}`",     inline=True)
        embed.add_field(
            name="総残高 LTC",
...
class BalanceOpModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with AsyncSessionLocal() as session:
            async with session.begin():
                ...
                if not db_user:
                    ...
                    # 同時作成による hd_index 重複の場合はロールバックして再取得
                        await session.rollback()
                        db_user = await session.get(User, str(uid_int))
                        if not db_user:
                            await interaction.followup.send(
                            "ユーザー作成中に競合が発生しました。もう一度お試しください。",
                                ephemeral=True
                            )
                            return

                old_avail  = quantize_ltc(db_user.available_balance or Decimal("0"))
                old_locked = quantize_ltc(db_user.locked_balance or Decimal("0"))
                ...
                new_avail  = quantize_ltc(db_user.available_balance or Decimal("0"))
                new_locked = quantize_ltc(db_user.locked_balance or Decimal("0"))
        ...
        embed = discord.Embed(title="残高操作完了", color=discord.Color.orange())
        embed.add_field(
            name="対象ユーザー", value=f"{display_name}\n(`{uid_str}`)", inline=False
        )
        embed.add_field(name="操作",     value=f"`{self.action}`",  inline=True)
        embed.add_field(
            name="入力金額",
            value=f"`{amount_dec:.8f}` LTC" if amount_dec > Decimal("0") else "—",
            inline=True,
        )
        embed.add_field(
            name="変更前",
...
class AdminDashboardView(discord.ui.View):
    ...
    async def fee_report(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            from models import Order, EscrowOrder

            collected_fee      = Decimal("0")
            withdraw_fee_total = Decimal("0")
            total_withdrawals  = 0
            total_deposits     = 0
            current_fee_pct    = Decimal("0")
            current_escrow_fee_pct = Decimal("0")
            total_p2p_orders   = 0
            completed_p2p      = 0
            ...
            async with AsyncSessionLocal() as session:
                ...
                config = res.scalar_one_or_none()
                if config:
                    collected_fee = quantize_ltc(config.collected_fees_ltc or Decimal("0"))
                    if config.p2p_fee_percent is not None:
                        current_fee_pct = Decimal(str(config.p2p_fee_percent)) * Decimal("100")
                    if config.escrow_fee_percent is not None:
                        current_escrow_fee_pct = Decimal(str(config.escrow_fee_percent)) * Decimal("100")

                w_fee = await session.execute(
                    select(func.coalesce(func.sum(Transaction.fee_ltc), 0))
                    .where(Transaction.tx_type == "WITHDRAW")
                )
                withdraw_fee_total = quantize_ltc(Decimal(str(w_fee.scalar() or 0)))
                ...
                d_cnt = await session.execute(
                    select(func.count()).select_from(Transaction)
                    .where(Transaction.tx_type == "DEPOSIT")
                )
                total_deposits = int(d_cnt.scalar() or 0)

                p2p_total_q = await session.execute(
                ...
                esc_total_q = await session.execute(
                    select(func.count()).select_from(EscrowOrder)
                )
                total_escrow_orders = int(esc_total_q.scalar() or 0)

                esc_done_q = await session.execute(
                    select(func.count()).select_from(EscrowOrder)
                    .where(EscrowOrder.status == "COMPLETED")
                )
            ...
            traceback.print_exc()
            await interaction.followup.send("レポート取得中にエラーが発生しました。", ephemeral=True)
...
class AdminCog(commands.Cog):
    ...
    async def admin_dashboard(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            ...
            description=(
                ...
                f"**現在のP2P手数料率:** `{current_fee * 100:.4f}%` "
                f"/ **仲介型:** `{current_escrow_fee * 100:.4f}%`"
            ),
            color=discord.Color.blurple(),
        )
        embed.add_field(name="手数料レポート",     value="収益・統計の一覧を確認します",          inline=True)
        embed.add_field(name="ユーザー情報確認",   value="ユーザーIDから残高・実績を確認します",   inline=True)
        embed.add_field(name="P2P手数料率を変更",  value="P2P取引にかかる手数料率(%)を変更します", inline=True)
        embed.add_field(name="仲介手数料率を変更", value="仲介型取引にかかる手数料率(%)を変更します", inline=True)
        embed.add_field(name="ユーザー残高を操作", value="特定ユーザーのLTC残高を直接操作します",  inline=True)
    ...
    @app_commands.command(
        name="setup_market",
        description="【管理者専用】P2P取引マーケットを設置します。"
    )
    ...
    @app_commands.command(
        ...
        description="【管理者専用】ユーザーダッシュボードを設置します。"
    )
    ...
    @app_commands.command(
        name="audit_ledger",
        description="【管理者専用】全ユーザーの台帳と現在残高の整合性をチェックします。"
    )
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                for user in all_users:
                    ledger_res = await session.execute(
                        select(func.coalesce(func.sum(Ledger.amount_ltc), 0))
                        .where(Ledger.user_id == user.discord_id)
                    )
                    ledger_sum = quantize_ltc(Decimal(str(ledger_res.scalar() or 0)))

                    actual_available = quantize_ltc(user.available_balance or Decimal("0"))
                    actual_locked    = quantize_ltc(user.locked_balance or Decimal("0"))
            ...
            if not mismatch_reports:
                embed = discord.Embed(
                    ...
                    description=(
                        f"全 {total_users_checked} ユーザーの残高と台帳が完全に一致しています。\n"
                ...
                embed = discord.Embed(
                    ...
                    description=(
                        f"ユーザー全体の期待残高: `{total_ledger:.8f} LTC`\n"
            ...
            traceback.print_exc()
            await interaction.followup.send("監査中にエラーが発生しました。", ephemeral=True)
