...
from utils.electrum import call_electrum_rpc, broadcast_withdrawal, SATOSHI
...
async def _check_escrow_deposits(bot):
    ...
    エスクローアドレスへの入金を getaddressunspent で確認する。
    """
    async with AsyncSessionLocal() as session:
        ...
        for order in orders:
            ...
            for utxo in unspent:
                value_raw = utxo.get("value", 0)
                h = utxo.get("height", 0)
                tx_hash = utxo.get("tx_hash", "")

                # ★ satoshi → LTC 変換
                val = _satoshi_to_ltc(value_raw)
                total_ltc += val
                if tx_hash:
                    latest_txid = tx_hash
            ...
            if total_ltc < required_ltc:
                if total_ltc > 0 and latest_txid != "N/A" and order.seller_sent_txid != latest_txid:
                    ...
                    logger.info(
                        f"EscrowOrder #{order.id}: 一部入金 "
                        f"({total_ltc:.8f} LTC, 不足 {shortage:.8f} LTC)"
                    )
            ...
            if confirmed_ltc >= required_ltc:
                if order.status != 'SELLER_LTC_CONFIRMED':
                    ...
                    await session.commit()
                    logger.info(
                        f"EscrowOrder #{order.id}: 着金確認完了 "
                        f"({confirmed_ltc:.8f} LTC, {REQUIRED_CONFIRMATIONS}承認)"
                    )
                ...
                if order.status != 'SELLER_LTC_DETECTED':
                    ...
                    await session.commit()
                    logger.info(
                        f"EscrowOrder #{order.id}: 入金検知（承認待ち）"
                        f"({total_ltc:.8f} LTC)"
                    )
...
async def _on_seller_ltc_partial(bot, order_id: int, detected: Decimal, shortage: Decimal, txid: str):
    ...
    await channel.send(
        ...
        f"TxID: `{txid}`\n"
        f"専用エスクローアドレスへ残りの不足額を追加で送金してください。"
    )


async def _on_seller_ltc_confirmed(bot, order_id: int):
    """LTC着金承認完了: WAITING_PAYMENT へ移行し買い手にビューを表示"""
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"**<@{buyer_id}> は下のボタンからLTC受取アドレスを登録し、**\n"
...
async def _check_seller_ltc_timeouts(bot):
    """WAITING_SELLER_LTC のまま seller_ltc_deadline を過ぎた注文を自動キャンセル"""
    ...
    async with AsyncSessionLocal() as session:
        ...
        for order in expired:
            ...
            if order.ticket_channel_id:
                ...
                if channel:
                    await channel.send(
                        "**売り手のLTC送金期限が切れたため、この取引は自動キャンセルされました。**\n"
                        "このチャンネルは10分後に削除されます。"
                    )