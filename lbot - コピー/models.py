...
class User(Base):
    ...
    unconfirmed_balance = Column(Numeric(16, 8), default=0.0)
    ...
    def total_balance(self):
        return self.available_balance + self.locked_balance + self.unconfirmed_balance


class Ledger(Base):
    """すべての残高変動の完全な履歴（複式簿記的台帳）"""
    __tablename__ = 'ledger'
    
    id = Column(Integer, primary_key=True, autoincrement=True)
    user_id = Column(String(20), ForeignKey('users.discord_id'), nullable=False)
    type = Column(String(20), nullable=False)  # DEPOSIT, WITHDRAW, BUY, SELL, FEE, ESCROW_LOCK, ESCROW_RELEASE, REFUND, ADMIN_ADJUST
    amount_ltc = Column(Numeric(16, 8), nullable=False)
    reference_id = Column(String(64), nullable=True)  # txid, order_id, etc.
...
class Order(Base):
    ...
    status = Column(String(20), default='PENDING')  # PENDING, PAID, RELEASING, COMPLETED, CANCELLED, APPEALED
...
class SystemConfig(Base):
    ...
    panel_message_ids = Column(Text, default='{}')  # JSON等でメッセージIDリストを保存


class EscrowAddress(Base):
    """仲介型用アドレスプール（新規取引には使い回さず、既存互換用）"""
...
async def get_p2p_fee_rate(session=None) -> Decimal:
    ...
    """
    DEFAULT_FEE = Decimal("0.002")  # フォールバック: 0.2%
