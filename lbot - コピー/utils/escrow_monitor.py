...
async def _check_escrow_deposits(bot):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for order in orders:
            ...
            # アドレスの残高を取得
            bal_data = await call_electrum_rpc("getaddressbalance", {"address": addr})
            if not bal_data or not isinstance(bal_data, dict):
                continue

            try:
                confirmed = Decimal(str(bal_data.get("confirmed", 0)))
                unconfirmed = Decimal(str(bal_data.get("unconfirmed", 0)))
                total_balance = confirmed + unconfirmed
            except Exception as e:
                logger.error(f"Failed to parse balance for {addr}: {e}")
                continue

            required_ltc = quantize_ltc(order.amount_ltc)
