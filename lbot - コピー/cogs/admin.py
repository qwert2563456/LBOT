...
class BalanceOpModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with AsyncSessionLocal() as session:
            db_user = await get_or_create_user(session, uid_int)
            old_avail  = float(db_user.available_balance)
            old_locked = float(db_user.locked_balance)
            ...
            if self.action == "zero":
                ...
                db_user.available_balance = old_avail + amount
            elif self.action == "sub_ltc":
                db_user.available_balance = max(0.0, old_avail - amount)

            new_avail  = float(db_user.available_balance)
            new_locked = float(db_user.locked_balance)
            await session.commit()
        ...
        embed.add_field(name="入力金額", value=f"`{amount:.8f}` LTC" if amount else "—", inline=True)
        embed.add_field(
            name="変更前",
