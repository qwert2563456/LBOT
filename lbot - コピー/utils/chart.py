...
def generate_chart_sync(data, tf_key: str) -> Optional[io.BytesIO]: 
    ...
    # ── 軸設定 ──
    days_val = int(TIMEFRAMES.get(tf_key, TIMEFRAMES["1h"]).get("days", "1"))
    tf_label = TIMEFRAMES.get(tf_key, TIMEFRAMES["1h"])["label"] 
    if days_val >= 14:
...
async def generate_chart(timeframe: str = "1h") -> Optional[io.BytesIO]:
    ...
    buf = generate_chart_sync(data, timeframe)
