    ...
    async def cancel_order(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with AsyncSessionLocal() as session:
            async with session.begin():
                ...
                total_refund = quantize_ltc(order.amount_ltc) + quantize_ltc(order.fee_ltc)
    ...
    async def release_ltc(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with AsyncSessionLocal() as session:
            async with session.begin():
                ...
                seller.locked_balance = quantize_ltc(seller.locked_balance) - total_locked
                seller.completed_trades += 1

                # 購入者へLTCを付与
                buyer.available_balance = quantize_ltc(buyer.available_balance) + amount_ltc
