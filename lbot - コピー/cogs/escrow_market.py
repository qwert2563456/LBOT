...
from utils.electrum import run_electrum_cmd
import json
...
async def refresh_escrow_market_panels(bot: commands.Bot, channel_id: int = None):
    ...
    async with escrow_market_refresh_lock:
        ...
        try:
            ...
            print(f"Failed to purge escrow market channel {target_id}")
    ...
    async with AsyncSessionLocal() as session:
        ...
        if not active_ads:
            ...
            await channel.send(embed=embed, view=EscrowMarketCommandView())
            return
        ...
        for ad in active_ads:
            ...
            # マージン適用後の価格計算
            applied_price = price_jpy * (float(ad.margin_percent) / 100.0)
            ...
            embed = discord.Embed(
                title=f"SELL LTC (仲介型) - {float(ad.margin_percent)}%",
        ...
        await channel.send(embed=embed_create, view=EscrowMarketCommandView())
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                async with session.begin():
                    ...
                    # 手数料を含めた売り手の送金必要額を計算
                    net_ltc = round(jpy / applied_price, 8)
                    fee_ltc = round(net_ltc * fee_rate, 8)
                    amount_ltc = round(net_ltc + fee_ltc, 8)  # 売り手が送る総額
                    ...
                    if pool_addr:
                        ...
                        # プールに空きがないので新規作成
                        new_addr_raw = await run_electrum_cmd("createnewaddress")
                        if not new_addr_raw:
                            raise ValueError("エスクローアドレスの生成に失敗しました。管理者に連絡してください。")

                        try:
                            parsed = json.loads(new_addr_raw)
                            escrow_address = parsed if isinstance(parsed, str) else new_addr_raw
                        except (json.JSONDecodeError, TypeError):
                            escrow_address = new_addr_raw.strip()
                    ...
                    await run_electrum_cmd("setlabel", escrow_address, escrow_label)
