...
class WithdrawModal(discord.ui.Modal, title="LTCの出金"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            fee_jpy = int(float(user_fee_dec) * price_jpy)
            ...
            async with AsyncSessionLocal() as session:
                ...
                # 残高が1円未満なら自動で0にする
                if new_balance > Decimal("0") and (float(new_balance) * price_jpy) < 1.0:
...
class DashboardView(discord.ui.View):
    ...
    async def withdraw(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        price_jpy = await fetch_ltc_jpy_price()
        max_jpy = int(float(available) * price_jpy) if price_jpy > 0 else 0
