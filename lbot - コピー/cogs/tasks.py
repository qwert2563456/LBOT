...
class TasksCog(commands.Cog):
    ...
    async def check_expired_orders(self):
        ...
        async with AsyncSessionLocal() as session:
            ...
            stmt_escrow_timeout = select(EscrowOrder).where(
                EscrowOrder.status == 'WAITING_PAYMENT',
                EscrowOrder.expires_at < func.now()
            )
            expired_escrow = (await session.execute(stmt_escrow_timeout)).scalars().all()

            for order in expired_escrow:
                async with session.begin_nested():
                    # 状態更新
                    order_to_cancel = await session.get(EscrowOrder, order.id, with_for_update=True)
            ...
            # --- 5. 仲介型エスクロー取引の警告処理 ---
            stmt_escrow_warning = select(EscrowOrder).where(
                EscrowOrder.status == 'WAITING_PAYMENT',
                EscrowOrder.warned_timeout == False,
                EscrowOrder.expires_at < func.now() + datetime.timedelta(minutes=5)
            )
            warning_escrow = (await session.execute(stmt_escrow_warning)).scalars().all()

            for order in warning_escrow:
                async with session.begin_nested():
                    order_to_warn = await session.get(EscrowOrder, order.id, with_for_update=True)
                    if order_to_warn and not order_to_warn.warned_timeout:
                        order_to_warn.warned_timeout = True
                        if order_to_warn.ticket_channel_id:
                            ticket_channel = self.bot.get_channel(int(order_to_warn.ticket_channel_id))
            ...
            # --- 6. 仲介型エスクローチケット自動削除 ---
            stmt_escrow_delete = select(EscrowOrder).where(
                EscrowOrder.ticket_delete_at.is_not(None),
                EscrowOrder.ticket_delete_at < func.now()
            )
            delete_escrow = (await session.execute(stmt_escrow_delete)).scalars().all()

            for order in delete_escrow:
                async with session.begin_nested():
                    order_to_delete = await session.get(EscrowOrder, order.id, with_for_update=True)
                    if not order_to_delete or not order_to_delete.ticket_delete_at:
                        continue
