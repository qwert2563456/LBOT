...
async def refresh_market_panels(bot: commands.Bot, channel_id: int = None):
    """
    マーケットチャンネルの広告パネルを再生成する。
    Rate Limit回避のため、既存メッセージの `edit` を優先的に使用する。
    """
    ...
    async with market_refresh_lock:
        ...
        async with AsyncSessionLocal() as session:
            ...
            if not active_ads:
                ...
                panels_to_show.append({"embed": embed, "view": MarketCommandView()})
            else:
                for ad in active_ads:
                    seller = ad.user
                    available_ltc = quantize_ltc(seller.available_balance)
                    ...
                    seller_available_jpy = int(available_ltc * Decimal(str(price_jpy)))

                    # 手数料を考慮した最大販売可能額
                    max_safe_ltc = available_ltc / (Decimal("1") + fee_rate)
                    max_safe_jpy = int(max_safe_ltc * Decimal(str(price_jpy)) * (ad.margin_percent / Decimal("100")))
                    ...
                    embed = discord.Embed(
                        title=f"SELL LTC  —  {ad.margin_percent.normalize()}% of market",
                    ...
                    embed.add_field(
                        ...
                        value=(
                            ...
                            f"**プラットフォーム手数料:** `{(fee_rate * Decimal('100')).normalize()}%`"
                        ),
                        inline=False,
                    )
                ...
                panels_to_show.append({"embed": embed_create, "view": MarketCommandView()})

        # ── 既存メッセージの書き換え(edit)または追加/削除 ──
        try:
            # チャンネルの過去メッセージを最大100件取得し、Botのメッセージを古い順に並べる
            history = [m async for m in channel.history(limit=100) if m.author == bot.user]
            history.reverse() # 古い順（上から順）に処理するため

            # 新しいパネルの方が多い場合は、editした後にsend
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                async with session.begin():
                    ...
                    applied_price    = Decimal(str(price_jpy_market)) * (ad.margin_percent / Decimal("100"))
                    required_ltc     = quantize_ltc(Decimal(str(jpy)) / Decimal(str(applied_price)))
                    fee_ltc          = quantize_ltc(required_ltc * fee_rate)
                    total_required   = required_ltc + fee_ltc
                    ...
                    if quantize_ltc(seller.available_balance) < total_required:
                        raise ValueError(
                            "販売者の残高が不足しているため、この金額での取引は現在開始できません。"
                        )

                    # 残高をロック: available → locked
                    seller.available_balance = quantize_ltc(seller.available_balance) - total_required
                    seller.locked_balance = quantize_ltc(seller.locked_balance) + total_required
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            async with AsyncSessionLocal() as session:
                ...
                max_safe_ltc   = available_ltc / (Decimal("1") + fee_rate)
                max_possible_jpy = int(max_safe_ltc * Decimal(str(price_jpy)) * (margin_val / Decimal("100")))

                if max_possible_jpy < min_val:
                    raise ValueError(
                        f"残高が不足しています。現在の残高({available_ltc} LTC)では "
...
class MarketCog(commands.Cog):
    ...
    async def cog_load(self):
        """再起動後にマーケットパネルを再生成してViewを正しく再登録"""
