    ...
    async def release_ltc(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with AsyncSessionLocal() as session:
            async with session.begin():
                ...
                if not txid:
                    await interaction.followup.send(
                        "LTC送金に失敗しました。Electrumデーモンの状態を確認してください。管理者に連絡してください。",
                        ephemeral=True
                    )
                    return
                ...
                if not config:
                    ...
                    session.add(config)
                config.collected_fees_ltc = round(float(config.collected_fees_ltc) + fee_ltc, 8)
    ...
    async def force_release(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with AsyncSessionLocal() as session:
            async with session.begin():
                ...
                # 1%の追加手数料を控除する処理
                penalty_fee_ltc = round(float(order.amount_ltc) * 0.01, 8)
                payout_ltc = round(net_ltc - penalty_fee_ltc, 8)
                
                if payout_ltc <= 0.0001:
                ...
                config.collected_fees_ltc = round(float(config.collected_fees_ltc) + fee_ltc + penalty_fee_ltc, 8)

                tx_b = Transaction(user_id=self.buyer_id, tx_type="ESCROW_BUY_ADMIN", txid=str(txid), amount_ltc=payout_ltc, fee_ltc=0.0, confirmations=1)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with AsyncSessionLocal() as session:
            async with session.begin():
                ...
                # 1%のペナルティ手数料を控除する
                penalty_fee_ltc = round(float(order.amount_ltc) * 0.01, 8)
                refund_ltc = round(float(order.amount_ltc) - penalty_fee_ltc, 8)
                refund_addr = self.seller_address.value.strip()

                if refund_ltc <= 0.0001:
                ...
                if not config:
                    ...
                    session.add(config)
                # システムの収益として加算
                config.collected_fees_ltc = round(float(config.collected_fees_ltc) + penalty_fee_ltc, 8)
